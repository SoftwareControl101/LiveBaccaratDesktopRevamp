...
public class PayoutTest19 extends Payout implements PayoutCase {
    ...
    public void setBetOption() {
        ...
        if (Helper.find(17, testCaseList)) return;
        if (Helper.find(18, testCaseList)) return;
    ...
    public void getBetOption() {
        if (!Helper.find(testCase, testCaseList) &&
                ...
                !Helper.find(18, testCaseList)) return;
    ...
    public void computeTestCase(String[] roundResult) {
        ...
        if (TestConditions.isPlayerDragonBonusDifferenceWin(roundResult, 4)) {
            payout = playerBet + (playerBet * payoutOdds);
            addWin(playerBet, payoutOdds);
        }

        if (TestConditions.isBankerDragonBonusDifferenceWin(roundResult, 4)) {
    ...
    public void saveTestCase(String[] roundResult) {
        if (!Helper.find(testCase, testCaseList)) return;
        if (!TestConditions.isPlayerDragonBonusDifferenceWin(roundResult, 4) &&
                !TestConditions.isBankerDragonBonusDifferenceWin(roundResult, 4)) return;
