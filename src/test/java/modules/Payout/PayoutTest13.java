...
public class PayoutTest13 extends Payout implements PayoutCase {
    ...
    private final int testCase = 13;
    private double playerBet, bankerBet, payout;
    private final double payoutOdds = 0;
    ...
    public void computeTestCase(String[] roundResult) {
        ...
        if (TestConditions.isNaturalTie(roundResult)) {
    ...
    public void saveTestCase(String[] roundResult) {
        if (!Helper.find(testCase, testCaseList)) return;
        if (!TestConditions.isNaturalTie(roundResult)) return;
