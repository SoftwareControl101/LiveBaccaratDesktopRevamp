...
public class PayoutTest18 extends Payout implements PayoutCase {
    ...
    private final int testCase = 18;
    private double playerBet, bankerBet, payout;
    private final double payoutOdds = 2;
    ...
    public void computeTestCase(String[] roundResult) {
        ...
        if (TestConditions.isPlayerDragonBonusDifferenceWin(roundResult, 5)) {
            payout = playerBet + (playerBet * payoutOdds);
            addWin(playerBet, payoutOdds);
        }

        if (TestConditions.isBankerDragonBonusDifferenceWin(roundResult, 5)) {
    ...
    public void saveTestCase(String[] roundResult) {
        if (!Helper.find(testCase, testCaseList)) return;
        if (!TestConditions.isPlayerDragonBonusDifferenceWin(roundResult, 5) &&
                !TestConditions.isBankerDragonBonusDifferenceWin(roundResult, 5)) return;
